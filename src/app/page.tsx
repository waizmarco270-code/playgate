...
const Header = ({
    ...
    onFiltered: (filteredVideos: VideoFile[], isSelectionMode: boolean, selectedVideoIds: Set<string>, isAddToPlaylistOpen: boolean, isBulkDeleteDialogOpen: boolean) => void;
}) => {
    ...
    useEffect(() => {
        onFiltered(filteredVideos, isSelectionMode, selectedVideoIds, isAddToPlaylistOpen, isBulkDeleteDialogOpen);
    }, [videos, searchTerm, isSelectionMode, selectedVideoIds, isAddToPlaylistOpen, isBulkDeleteDialogOpen, onFiltered, filteredVideos]);
    ...
    const handleBulkDelete = async () => {
        try {
          ...
          // The parent will handle the state update via loadVideos
           toast({
        ...
        <div className="flex flex-col h-full">
            <header className="relative flex items-center justify-between p-4 border-b w-full overflow-hidden h-[73px]">
             ...
             <main className="flex-1 p-6 overflow-y-auto">
                {videos.length > 0 && filteredVideos.length === 0 && searchTerm ? (
                    ...
                    <VideoGrid>
                        <VideoGrid.Content 
                            ...
                            onVideoDeleted={() => {}} // This is handled by parent
                            onVideoUpdated={() => {}} // This is handled by parent
                            context="library"
...
export default function HomePage() {
  ...
  const handleUpdateFromHeader = useCallback((
      newFilteredVideos: VideoFile[], 
  ...
  // These functions now just reload the video list
  ...
  return (
    <>
    <div className="flex flex-col h-full">
      <Header videos={allVideos} handleImportClick={handleImportClick} onFiltered={handleUpdateFromHeader} />
       <input
          type="file"
          ref={fileInputRef}
      ...
      {loading ? (
          ...
          <main className="flex-1 p-6 overflow-y-auto">
            ...
            // The Header component now handles the grid and the "No Results" message
            null
        )}
    </div>
    </>
